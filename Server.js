...
function startServer() {
    ...
    app.get(['/dashboard/add-didar'], (req, res) => {
    ...
    app.post(['/dashboard/add-didar'], async (req, res) => {
        try {
            const { name, date, time, invited , presenter} = checkXSS(req.body);
    ...
    app.post(['/users/register'], async (req, res) => {
        try {
            ...
            return res.status(200).json("عضویت با موفقیت انجام شد");
        } catch (err) {
            console.log(err);
    ...
    app.post(['/login'], async (req, res) => {
        ...
        try {
            ...
            var didar = [];
            didar = await Didar.find({presenter: username});
            var name1 = [didar.length];
            for(let i=0; i<didar.length; i++){
                
                name1[i] = didar[i].name;
            }
            console.log(name1);
            if(isMatch){
                ...
                hostCfg.authenticated = true;
                res.status(200).json({ message: {token: token, username: username, fullname: fullname, name: name1} });
            } else {
